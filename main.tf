...
resource "aws_vpc_security_group_ingress_rule" "allow_cloudflare_http" {
  for_each = toset(compact(split("\n", file("cloudflare-ips.txt"))))

  security_group_id = aws_security_group.allow_cloudflare_http.id
...
resource "aws_instance" "ec2-instance" {
  ...
  vpc_security_group_ids      = [aws_security_group.allow_ssh.id, aws_security_group.allow_cloudflare_http.id, aws_security_group.allow_all_egress.id]
